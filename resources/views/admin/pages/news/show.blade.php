                            ...
                            <form method="post" action="{{route('admin.news.update', $news->id)}}" enctype="multipart/form-data">
                                ...
                                <div class="form-row">
                                    @if($locale)
                                        @foreach($locale as $l)
                                            <div class="col-md-12">
                                                <div class="form-group"><label class="small mb-1" for="inputFirstName"><strong>Xəbər başlığı ({{$l->code}}) :</strong></label><input class="form-control py-4" type="text" placeholder="xəbər başlığını daxil edin" name="title[{{$l->code}}]" value="{{($news->translations($l->code)) ? $news->translations($l->code)->title : null}}" /></div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group"><label class="small mb-1" for="inputFirstName"><strong>Xəbər mətni ({{$l->code}}):</strong></label><textarea class="form-control ckeditor" placeholder="Xəbər mətnini daxil edin" name="description[{{$l->code}}]">{{($news->translations($l->code)) ? $news->translations($l->code)->description : null}}</textarea></div>
                                            </div>
                                        @endforeach
                                    @endif

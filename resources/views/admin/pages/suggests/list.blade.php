                    ...
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="home-tab" data-toggle="tab" href="#unread" role="tab" aria-controls="home" aria-selected="true">
                                Oxunmayanlar
                                @if(count($uread) > 0)
                                    <kbd class="bg-primary ml-1">{{count($uread)}}</kbd>
                                @endif
                            </a>
                        </li>
                        <li class="nav-item">
                                ...
                                @if($uread)
                                    @foreach($uread as $i=>$u)
                                        <tr>
                                            ...
                                            <td>{{$u->email}}</td>
                                            <td>
                                                <a href="{{route('admin.suggests.show',$u->id)}}" title="Bax" class="p-1 bg-success text-white"><i class="far fa-eye"></i></a>
                                                <a href="{{route('admin.suggests.destroy',$u->id)}}" title="Sil" class="p-1 bg-danger text-white"><i class="far fa-trash-alt"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
                                ...
                                @if($rea)
                                    @foreach($rea as $i=>$r)
                                        <tr>
                                            ...
                                            <td>{{$r->email}}</td>
                                            <td>
                                                <a href="{{route('admin.suggests.show',$r->id)}}" title="Bax" class="p-1 bg-success text-white"><i class="far fa-eye"></i></a>
                                                <a href="{{route('admin.suggests.destroy',$r->id)}}" title="Sil" class="p-1 bg-danger text-white"><i class="far fa-trash-alt"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
